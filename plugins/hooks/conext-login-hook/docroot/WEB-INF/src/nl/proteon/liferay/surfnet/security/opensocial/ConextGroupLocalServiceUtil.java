...
public class ConextGroupLocalServiceUtil {

	/**
	 * Update groups based on the groups of the user from the IDP
	 * 
	 * @param companyId
	 * @param user
	 */
	public static void updateGroups(long companyId, User user,
			String accessToken) {

		if (user != null) {
			try {

				List<OpenSocialGroup> openSocialGroups = OAuth2ServiceUtil
						.getGroups(accessToken, user.getOpenId());

				for (OpenSocialGroup openSocialGroup : openSocialGroups) {
					Group group = null;
					group = getGroup(companyId, openSocialGroup.getTitle());
					if (group == null) {
						_log.info("Group " + openSocialGroup.getId()
								+ " is new");
						group = addGroup(
								user.getUserId(),
								companyId,
						...
						_log.info("Group " + openSocialGroup.getId()
								+ " added as " + group.getName());

						Layout layout = LayoutLocalServiceUtil.addLayout(
								user.getUserId(), group.getGroupId(), true, -1,
								"our_page", "our_title", "",
						...
						LayoutTypePortlet layoutTypePortlet = (LayoutTypePortlet) layout
								.getLayoutType();
						layoutTypePortlet.setLayoutTemplateId(user.getUserId(),
								"1_column");
						String documentLibraryPortletId = layoutTypePortlet
								.addPortletId(user.getUserId(),
						...
						PortletPreferences prefs = PortletPreferencesLocalServiceUtil
								.getPreferences(companyId, ownerId, ownerType,
										layout.getPlid(),
										documentLibraryPortletId);

						prefs.setValue("portletSetupShowBorders", "false");

						PortletPreferencesLocalServiceUtil.updatePreferences(
								ownerId, ownerType, layout.getPlid(),
								documentLibraryPortletId, prefs);
						...
						_log.info("Group " + openSocialGroup.getId()
								+ " already exists");
						group = updateGroup(companyId, group.getGroupId(),
								openSocialGroup.getDescription());
					}
					...
					try {
						...
						_log.info("Set membership for " + user.getOpenId()
								+ " on " + group.getName());
				...
				for (OpenSocialGroup openSocialGroup : openSocialGroups) {
					openSocialGroupsMap.put(openSocialGroup.getTitle(),
							openSocialGroup);
				}
				...
				for (Group group : groups) {
					if (openSocialGroupsMap.get(group.getName()) == null) {
						GroupLocalServiceUtil.unsetUserGroups(user.getUserId(),
								new long[] { group.getGroupId() });
						_log.info("User " + user.getOpenId() + " removed from "
								+ group.getName());
					}
				}
	...
	private static Group addGroup(long userId, long companyId, String name,
			String description, String friendlyURL) {
		Group group = null;
		try {
			...
			group = GroupLocalServiceUtil.addGroup(userId, className, classPK,
	...
	private static Group getGroup(long companyId, String name) {
		Group group = null;
		try {
			group = GroupLocalServiceUtil.getGroup(companyId, name);
		} catch (Exception e) {
			// no problem, probably a new group
		}
		return group;
	}

	private static Group updateGroup(long companyId, long groupId,
			String description) {
		Group group = null;
		try {
	...
	private static Log _log = LogFactoryUtil
			.getLog(ConextGroupLocalServiceUtil.class);
}
